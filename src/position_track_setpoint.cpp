...
const double landing_threshold = 0.05;
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO("Listening to apriltag detection topic: %s", tag_36h11_detection_topic.c_str());
  ROS_INFO("Listening to apriltag detection topic: %s", tag_16h5_detection_topic.c_str());
  ...
  error_yaw_pub = nh.advertise<std_msgs::Float64>("/teamhku/position_track/yaw_state", 10);
  ...
  std_msgs::Float64 error_yaw_msg;
  ...
  while (ros::ok())
  {
    ...
    if(found_16h5 || found_36h11)
    {
      ...
      for(int i = 0; i < tag_16h5_num; i++)
        {
          if(tags_16h5[i]->isFound())
          {
            ...
            // std::cout << "id: " << i << " self position x: " << tags_16h5[i]->position_drone_frame_(0) << " y: " << tags_16h5[i]->position_drone_frame_(1) << " z: " << tags_16h5[i]->position_drone_frame_(2) << " landing center x: " << landing_center_position(0) << "y: " << landing_center_position(1) << "z: " << landing_center_position(2) << std::endl;
          }
        }
      ...
      //   if(tags_16h5[i]->isFound())
      //     {
      //       // std::cout << "id: " << i << " orientation drone frame:  w: \t" << tags_16h5[i]->orientation_drone_frame_.w() << " x: \t" << tags_16h5[i]->orientation_drone_frame_.x() << " y: \t" << tags_16h5[i]->orientation_drone_frame_.y() << " z: \t" << tags_16h5[i]->orientation_drone_frame_.z() <<std::endl;
      //     }
      // }
